  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: "Заявку на зварювання передано в обробку. Перенаправляємо на сторінку подяки...",
      });

      persistAttribution({ source: 'metal_welding', utm_source: utmSource, utm_medium: utmMedium, utm_campaign: utmCampaign });
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="py-20">
        <div className="container mx-auto px-4">
          ...
          <GalleryLightbox images={galleryImages.map(i => ({ id: i.id, src: i.img, alt: `Приклад ${i.id}` }))} />
        </div>
      </section>
          ...
          <div className="max-w-4xl mx-auto">
            <Card className="p-6 md:p-8 shadow-2xl border-primary/20 bg-gradient-to-br from-card to-background">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {/* Параметри зварювання - всі необов'язкові */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-foreground flex items-center gap-2">
                      Тип зварювання (необов'язково)
                    </label>
                    <select
                      name="weldingType"
                ...
                {/* Статус відправки - прибираємо блок success, залишаємо тільки error */}
                {submitStatus === 'error' && (
